...
void inv_icm20948_sleep_us(int us){
	...
	{
		if(htim7.Instance->CNT - tickstart < 0){
			tickstart -= htim7.Instance->ARR;
		}
	}
}
...
void check_rc(int rc, const char * msg_context){
	if(rc < 0) {
		...
		while(1){
			INV_MSG(INV_MSG_LEVEL_INFO, "-------Resetting chip-------");
			chip_reset();
		}
	}
}
...
extern void icm_data_process(inv_sensor_event_t *);
