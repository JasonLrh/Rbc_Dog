...
// }
extern osMessageQId qSerialHandle;

uint8_t *trans_buffer = NULL;
void dog_cmd_rx_callback(UART_HandleTypeDef *huart, uint16_t pos)
{
    trans_buffer[pos] = '\0';
    xQueueSendFromISR(qSerialHandle, &trans_buffer, NULL);
    trans_buffer = pvPortMalloc(256);
    if (trans_buffer != NULL)
    {
        assert_param(HAL_UARTEx_ReceiveToIdle_IT(huart, trans_buffer, 256) == HAL_OK);
    }
}
...
void SerialCmdTask(void const *argument)
{
    ...
    for (;;)
    {
        uint8_t *buffer_point[1];
        uint8_t *buffer;

        if (xQueueReceive(qSerialHandle, buffer_point, portMAX_DELAY) == pdTRUE)
        {
            buffer = *buffer_point;
            uart_printf("$ %s\n", buffer); // input string
            // TODO process cmd string here
            uint16_t pos;
            ...
            if (pos > 0)
            {
                ...
                uart_printf("[cmd] $ %s", buffer);
                switch (buffer[0])
                {
                case 'M':
                {
                    config_motors((const char *)(buffer + 1));
                }
                break;
                ...
                case 'S':
                {
                    config_system((const char *)(buffer + 1));
                }
